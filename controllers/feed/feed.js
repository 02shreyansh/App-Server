...
const getReelPosts = async (req, res) => {
    ...
    try {
        const myReelPosts = await Reels.find({ user: userId })
            .sort({ createdAt: -1 })
            .skip(parseInt(offset))
            .limit(parseInt(limit))
    ...
    try {
        ...
        for (const reelId of reelIds) {
            if (!userHistory.reels.some((r) => r.reel.toString() === reelId)) {
                ...
                await Reels.findByIdAndUpdate(reelId, { $inc: { viewCount: 1 } });
            }
        }
...
const getHomeFeed = async (req, res) => {
    ...
    try {
        ...
        if (totalReels < limit + offset) {
            ...
            const mostLikedReels = await Reels.aggregate([
